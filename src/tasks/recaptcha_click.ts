...
export class RecaptchaClickTask extends UAProxy {
    /**
     * Initialize a new RecaptchaClickTask for handling reCAPTCHA click-based captchas
     ...
     * @param task {@link IRecaptchaClickTaskRequest}
     * @returns Only the task you created {@link IRecaptchaClickTaskRequest}
     */
    public task = (
        task: Omit<IRecaptchaClickTaskRequest, "type"> & { proxy?: ProxyConfig }
    ) => task

    /**
     * Creates a reCAPTCHA click task for solving
    ...
    public createWithTask = async (
        ...
        const data: IRecaptchaClickTaskRequest = {
    ...
    public getTaskResult = async (taskId: number) =>
        this._getTaskResult<IRecaptchaClickTaskResponse>(taskId)
    ...
    public joinTaskResult = async (taskId: number, timeout?: number) =>
        this._joinTaskResult<IRecaptchaClickTaskResponse>(taskId, timeout)
}

interface IRecaptchaClickTaskRequest extends ITask {
    type: "RecaptchaClickTask" | "RecaptchaClickTaskProxyless"
    /**
...
interface IRecaptchaClickTaskResponse extends ITaskSolution {}
